...
- Audio changes must keep the unlock dance, the iPad audio-session promotion (audioSession.type='playback' + silent <audio> keepalive, Apple-gated and >=1s — un-gated it crackles on Android). fluid.html keeps its clock-based polyphony budget (camera load), plain flux does not, AND the activity-gating of the pad (ambient beds must fade out when idle — user-mandated across the series).
